...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap<K, M, H>& rhs) noexcept :
    _size(0),
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap&& rhs) noexcept :
...
template <typename K, typename M, typename H>
HashMap<K, M, H>& HashMap<K, M, H>::operator=(const HashMap<K, M, H>& rhs) noexcept {
    if (this != &rhs) {
        this->clear();
        _buckets_array.clear();
        _buckets_array.resize(rhs.bucket_count(), nullptr);
        for(auto iter = rhs.begin(); iter != rhs.end(); ++iter)
            insert(*iter);
    }
    return *this;
...
const M& HashMap<K, M, H>::at(const K& key) const {
    // using static_cast/const_cast tricks.
    // note, here need cast to "const M&" rather than "const M"
...
void HashMap<K, M, H>::debug() const {
    const_cast<HashMap<K, M, H>*>(this)->debug();    
}
